...
class GenerationConfig(FairseqDataclass):
    ...
    kenlm_model: Optional[str] = field(
        default=None,
        metadata={"help": "path to kenlm  for lm fusion"},
    )
    ...
    # Joint CTC/Attn decoding
    ctc_infer_weight: float = field(
        default=0,
        metadata={"help": "joint ctc/attn decoding"}
    )
    # CTC inference
    ctc_infer: bool = field(
        default=False,
        metadata={"help": "generate CTC results during inference"}
    )
    ...
    # cal localness during inference
    cal_localness: bool = field(
        default=False,
        metadata={"help": "calculate attention localness during inference"}
    )
    localness_window: float = field(
        default=0.1,
        metadata={"help": "window size for the computation of attention localness during inference"}
    )
    # cal entropy during inference
    cal_entropy: bool = field(
        default=False,
        metadata={"help": "calculate entropy of attention weights during inference"}
    )
    # cal topk weights of cross attention during inference
    cal_topk_cross_attn_weights: bool = field(
        default=False,
        metadata={"help": "calculate topk weights of cross attention during inference"}
    )
    topk_cross_attn_weights: int = field(
        default=1,
        metadata={"help": "topk size for the statistics of cross attention weights during inference"}
    )
    # cal monotonic weights of cross attention during inference
    cal_monotonic_cross_attn_weights: bool = field(
        default=False,
        metadata={"help": "calculate monotonic weights of cross attention during inference"}
    )

    # cal cosine similarity of encoding representations
    gather_cos_sim: bool = field(
        default=False,
        metadata={"help": "gather cosine similarity of encoding representation during inference"}
    )
    gather_cos_sim_dis: int = field(
        default=2,
        metadata={"help": "the distance of gather cosine similarity of encoding representation during inference"}
    )
    mixup_infer: bool = field(
        default=False,
        metadata={"help": "apply mixup during inference"}
    )
    save_rep: bool = field(
        default=False,
        metadata={"help": "save the representation into a file during inference"}
    )
    cal_flops: bool = field(
        default=False,
        metadata={"help": "calculate GFLOPs during inference"}
    )
