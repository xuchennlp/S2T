...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if getattr(args, "encoder_freeze_module", None):
            ...
            logger.info("freeze the encoder module: {}".format(args.encoder_freeze_module))

        decoder = cls.build_decoder(args, tgt_dict, decoder_embed_tokens)
        if getattr(args, "decoder_freeze_module", None):
            utils.freeze_parameters(decoder, args.decoder_freeze_module)
            logger.info("freeze the decoder module: {}".format(args.decoder_freeze_module))
...
class TransformerDecoder(FairseqIncrementalDecoder):
        ...
        if self.save_rep:
            ...
            for key, value in x.items():
                ...
                if key in keep_idx:
                    ...
                    if key == 0:
                        title = "Input"
                    elif key == 7: # or key ==4 or key == 10:
                        title = "Output"
                    else:
                        title = ("Layer " + str(key))
                    plt.title(title, fontsize = 12) # change the fontsize of title

            save_path = os.path.join(path, "mixup_dec.eps")
            plt.savefig(save_path, dpi=600)
            save_path = os.path.join(path, "mixup_dec.png")
            plt.savefig(save_path, dpi=600)
    ...
    def add_rep(self, x, layer_idx, mixup=None):
        if mixup is None or not mixup["mixup_decoder_emb"]:
            self.save_rep = False
            return
        ...
        if mixup["mixup_decoder_emb"]:
            ...
            org_flag = mixup["decoder_org_flag"]
            mixup_flag1 = mixup["decoder_mixup_flag1"]
            mixup_flag2 = mixup["decoder_mixup_flag2"]

            if any(mixup_flag1):
                mixup_x1 = x[:, mixup_flag1, :].mean(0)
                mixup_x2 = x[:, mixup_flag2, :].mean(0)
                mixup_rep_dict[layer_idx].append(torch.stack([mixup_x1, mixup_x2], dim=0))
            
            if any(org_flag):
                org_x = x[:, org_flag, :].mean(0)
                rep_dict[layer_idx].append(org_x)
    ...
    def apply_mixup(self, encoder_out, x, self_attn_padding_mask):
        ...
        if mixup["mixup_decoder_emb"]:
            ...
            decoder_org_flag = ([1] * len(org_idx))
            ...
            decoder_org_flag.extend([0] * len(mix_idx1) * 2)
            ...
            mixup["decoder_org_flag"] = torch.Tensor(decoder_org_flag).to(x.device).bool()
