...
class S2TDataConfig(object):
    ...
    def __init__(self, yaml_path):
        ...
        self.config = {}
        self.yaml_dir = os.path.dirname(yaml_path)
    ...
    def get_feature_transforms(self, split, is_train):
        ...
        if self.config.get("stats_npz_path", None):
            cmvn_dict = {"stats_npz_path": self.config["stats_npz_path"]}
            self.config["global_cmvn"] = cmvn_dict
        ...
        if is_train and self.config.get("no_specaugment", False) and "specaugment" in cur:
            cur.remove("specaugment")
        if self.config.get("cmvn", None) and ("utterance_cmvn" in cur or "global_cmvn" in cur):
            cmvn_type = self.config.get("cmvn", None)
            if "utterance_cmvn" in cur:
                cur.remove("utterance_cmvn")
            if "global_cmvn" in cur:
                cur.remove("global_cmvn")
            if cmvn_type == "utterance":
                cur.append("utterance_cmvn")
            elif cmvn_type == "global":
                cur.append("global_cmvn")
                if self.config.get("cmvn_path", None):
                    cmvn_path = self.config["cmvn_path"]
                    if not os.path.exists(cmvn_path):
                        cmvn_path = os.path.join(self.yaml_dir, cmvn_path)

                    cmvn_dict = {"stats_npz_path": cmvn_path}
                    cfg["global_cmvn"] = cmvn_dict
                assert "global_cmvn" in cfg and os.path.exists(cfg["global_cmvn"]["stats_npz_path"])

        if "utterance_cmvn" in cur:
            cfg["utterance_cmvn"] = dict()
        if self.config.get("cmvn_no_axis", False):
            cfg["utterance_cmvn"]["cmvn_no_axis"] = True

        if "specaugment" in cur and self.config.get("overwrite_specaug", None) is not None:
            for key in ["freq_mask_F", "freq_mask_N", "time_mask_N", "time_mask_T", "time_mask_P", "time_warp_W"]:
                overwrite_value = self.config["overwrite_specaug"][key]
                if overwrite_value is not None:
                    cfg["specaugment"][key] = overwrite_value
        cfg["transforms"] = cur
        return cfg
