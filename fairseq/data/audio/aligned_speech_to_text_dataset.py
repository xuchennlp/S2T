...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        aligned_tgt_texts: Optional[List[str]] = None,
        ctc_tgt_texts: Optional[List[str]] = None,
    ...
    def __getitem__(
        ...
        ctc_target = None
        if self.ctc_tgt_texts is not None:
        ...
        return index, source, target, aligned_target, ctc_target, transcript

    def __len__(self):
        return self.n_samples

    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]]) -> Dict:
        if len(samples) == 0:
            return {}
        ...
        if self.src_dict is not None and self.src_texts is not None:
            ...
            transcript_lengths = torch.tensor(
                [t.size(0) for _, _, _, _, _, t in samples], dtype=torch.long
            ).index_select(0, order)
            transcript_ntokens = sum(t.size(0) for _, _, _, _, _, t in samples)
        else:
            transcript = None
