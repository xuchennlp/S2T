...
class SpeechToTextDataset(FairseqDataset):
    ...
    def tokenize_text(self, text: str, bpe_tokenizer):
        if self.pre_tokenizer is not None:
            text = self.pre_tokenizer.encode(text)
    ...
    def __getitem__(
        self, index: int
    ) -> Tuple[int, torch.Tensor, Optional[list]]:
        ...
        if self.tgt_texts is not None:
            tokenized = self.tokenize_text(self.tgt_texts[index], self.bpe_tokenizer)
        ...
        if self.src_dict is not None and self.src_texts is not None and self.src_bpe_tokenizer is not None:
            assert isinstance(self.src_dict, list) and isinstance(self.src_bpe_tokenizer, list)
            assert len(self.src_dict) == len(self.src_bpe_tokenizer)

            for dict, bpe in zip(self.src_dict, self.src_bpe_tokenizer):
                tokenized = self.tokenize_text(self.src_texts[index], bpe)
                transcript = dict.encode_line(
                    tokenized, add_if_not_exist=False, append_eos=True
                ).long()
                mlo_transcript.append(transcript)

        return index, source, target, mlo_transcript

    def __len__(self):
        return self.n_samples

    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor, list]]) -> Dict:
        if len(samples) == 0:
            return {}
        ...
        if self.src_dict is not None and self.src_texts is not None:
            ...
            for i in range(dict_num):
                ...
                transcript_lengths = torch.tensor(
                    [t[i].size(0) for _, _, _, t in samples], dtype=torch.long
                ).index_select(0, order)
                transcript_ntokens = sum(t[i].size(0) for _, _, _, t in samples)
                mlo_transcript.append({
                    "tokens": transcript,
        ...
        if len(mlo_transcript) > 1:
            for i in range(len(mlo_transcript)):
                idx = i + 1
                out["transcript%d" % idx] = mlo_transcript[i]
