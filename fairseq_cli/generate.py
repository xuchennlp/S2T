...
def _main(cfg: DictConfig, output_file, translation_path=None):
    ...
    for model in models:
        ...
        if hasattr(model, "encoder") and hasattr(model.encoder, "set_flag"):                            
            model.encoder.set_flag(
                ...
                cal_entropy=cfg.generation.cal_entropy, 
                cal_topk_cross_attn_weights=cfg.generation.cal_topk_cross_attn_weights, 
                topk_cross_attn_weights=cfg.generation.topk_cross_attn_weights,
                save_rep=cfg.generation.save_rep,
        ...
        if hasattr(model, "decoder") and hasattr(model.decoder, "set_flag"):                            
            model.decoder.set_flag(
                ...
                cal_topk_cross_attn_weights=cfg.generation.cal_topk_cross_attn_weights, 
                topk_cross_attn_weights=cfg.generation.topk_cross_attn_weights,
                cal_monotonic_cross_attn_weights=cfg.generation.cal_monotonic_cross_attn_weights, 
                save_rep=cfg.generation.save_rep,
                mixup_infer=cfg.generation.mixup_infer,
                )
    ...
    for model in models:
        if hasattr(model, "encoder") and hasattr(model.encoder, "dump"):                            
            if cfg.common_eval.results_path is not None:
                with open(os.path.join(cfg.common_eval.results_path, "enc_dump"), "w", encoding="utf-8") as fw:
                    model.encoder.dump(fw, "Encoder")
            else:
                model.encoder.dump(sys.stdout, "Encoder")

        if hasattr(model, "decoder") and hasattr(model.decoder, "dump"):                            
            if cfg.common_eval.results_path is not None:
                with open(os.path.join(cfg.common_eval.results_path, "dec_dump"), "w", encoding="utf-8") as fw:
                    model.decoder.dump(fw, "Decoder")
            else:
                model.decoder.dump(sys.stdout, "Decoder")
    ...
    if hasattr(models[0], "decoder") and models[0].decoder.gather_attn_weight:
        ...
        if cfg.common_eval.results_path is not None:
            with open(os.path.join(cfg.common_eval.results_path, "weights"), "w", encoding="utf-8") as fw:
